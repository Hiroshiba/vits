...
from text import text_to_sequence, cleaned_text_to_sequence, accent_to_sequence
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        for audiopath, sid, text, a_s, a_e, a_p_s, a_p_e in self.audiopaths_sid_text:
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                audiopaths_sid_text_new.append([audiopath, sid, text, a_s, a_e, a_p_s, a_p_e])
                lengths.append(os.path.getsize(audiopath) // (2 * self.hop_length))
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        a_s, a_e, a_p_s, a_p_e = self.get_accent(a_s.split()), self.get_accent(a_e.split()), self.get_accent(a_p_s.split()), self.get_accent(a_p_e.split())
        spec, wav = self.get_audio(audiopath)
    ...
    def get_audio(self, filename):
        ...
        if sampling_rate != self.sampling_rate:
            raise ValueError("{} {} SR doesn't match target {} SR".format(
                sampling_rate, self.sampling_rate))
        audio_norm = audio_norm.unsqueeze(0)
        spec_filename = "/tmp/" + Path(filename).stem + ".spec.pt"
        if os.path.exists(spec_filename):
            spec = torch.load(spec_filename)
        else:
            spec = spectrogram_torch(audio_norm, self.filter_length,
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        if self.return_ids:
            return text_padded, a_s_padded, a_e_padded, a_p_s_padded, a_p_e_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, sid, ids_sorted_decreasing
        return text_padded, a_s_padded, a_e_padded, a_p_s_padded, a_p_e_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, sid
